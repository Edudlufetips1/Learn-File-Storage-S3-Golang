...
func (store *Store) VerifyAndConsume(ctx context.Context, userID int64, code, secret string) (bool, error) {
	currentTime := store.now().UTC()
	if !verifyAt(code, secret, currentTime) {
		return false, nil
	}
	timeStep := currentTime.Unix() / int64(totpPeriodSeconds)
	...
	if err != nil {
		return false, fmt.Errorf("count consumed TOTP steps: %w", err)
	}
...
func (store *Store) ConsumeBackupCode(ctx context.Context, userID int64, code string) (bool, error) {
	consumed, err := store.queries.ConsumeTOTPBackupCode(ctx, dbgen.ConsumeTOTPBackupCodeParams{
		UserID:   userID,
